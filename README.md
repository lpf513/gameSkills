...
| level-design | Layout single levels, maps, or missions | Textual row‑by‑row description, optional ASCII grid, pacing chart, encounter distribution table, player guidance plan |
...
| game-performance | Optimize CPU/GPU, memory, draw calls, frame‑rate, and loading times | Profiling guide, root‑cause hypothesis, concrete code change suggestions, performance budgets, change‑measure loop |

> **Note:** Each skill also includes a suggested `scripts/`, `references/`, and `assets/` directory layout (with example file names) so you can easily add reusable tools, templates, and reference material later.
...
2. **Invoke a skill** in a Codex conversation:  
   ...
   ```
   Use $game-design-doc to create a GDD for a sci‑fi RPG with three core pillars.
   ```
...
4. **Add your own resources** – Populate the optional `scripts/`, `references/`, and `assets/` sub‑folders with helpers, templates, or domain‑specific data to make the skill even more powerful for your team.

## 📖 Skill Structure (SKILL.md)

Every `SKILL.md` follows a consistent, professional template:

- **YAML Frontmatter** – `name` and rich `description` (the trigger for Codex).  
...
- **## Output Format** – Primary format (structured Markdown) plus optional alternatives (CSV/Excel tables, Mermaid diagrams, JSON schemas, pseudocode).  
- **## Resources (optional)** – Only keep sections for resources you actually create:  
  - `scripts/` – helper scripts (e.g., `generate_gdd.py`, `balance_calc.py`, `level_template.py`)  
  - `references/` – reference documents (e.g., `gdd_structure.md`, `system_design_patterns.md`, `monetization_models.csv`)  
  - `assets/` – template files, icons, or other assets (e.g., `gdd_template.docx`, icon set)  
...
- **Store reference material** – Design patterns, GDD outlines, monetization model tables, etc., go in `references/`.  
- **Include templates** – Word/Google‑Doc templates, icon sets, HTML skeletons, etc., belong in `assets/`.  
- **Adjust UI metadata** – Edit `agents/openai.yaml` to change the display name, short description, brand color, or default prompt if you want a different presentation in the Codex skill picker.  
- **Add new skills** – Follow the same `SKILL.md` + `agents/openai.yaml` structure; keep the language actionable, concise, and professional.
...
> **You**: `Use $game-design-doc to draft a GDD for a mobile puzzle‑match‑3 game with a “short session, deep mastery” pillar.`  
...
> **You**: `Use $game-balance to balance damage values for a swords‑vs‑sorcerers combat system.`  
...
> **You**: `Use $level-design to layout a three‑encounter dungeon for a fantasy roguelike.`  
...
When adding a new skill, please follow the same `SKILL.md` + `agents/openai.yaml` structure and keep the language actionable, concise, and professional.
...
| 技能 | 主要用途 | 典型输出 |
|------|----------|----------|
| game-design-doc | 创建完整的游戏设计文档（GDD） | 结构化 Markdown GDD（愿景、支柱、核心循环、系统交互矩阵、功能待办列表（优先级）、玩家角色、盈利、风险分析） |
| game-system-design | 设计特定游戏系统（战斗、经济、进度、社交等） | 系统规格说明、核心循环、反馈机制、集成点、调节旋钮、快速验证计划 |
| level-design | 设计单个关卡、地图或任务 | 文字房间‑by‑行描述、可选的 ASCII 网格、节奏图、遭遇分布表、玩家引导计划 |
| narrative-design | 编写故事、角色、对话和世界观 | 故事圣经、角色档案、对话脚本、情节流程图、环境小景 |
| game-balance | 通过数学建模与概率调整数值 | 公式、电子表格、模拟结果、平衡建议、红旗清单 |
| game-economy | 设计可持续的游戏内经济与 monetization（变现） | 来源/汇去图、转换率矩阵、定价模型、通胀/通缩控制、可持续性指标 |
| probability-model | 构建掉落、抽卡、暴击等随机模型 | 概率分布、期望值/方差表、置信区间、模拟脚本 |
| data-table-generator | 生成 CSV/Excel 表格（道具、敌人、技能、任务等） | 可直接导入的数据文件，包含公式、缩放曲线、方差控制、架构验证说明 |
| game-dev-arch | 定义技术架构（ECS vs OOP、网络、渲染、可扩展性） | 架构图、技术决策矩阵、子系统需求、资产管道描述 |
| game-feature-impl | 将设计转化为实施计划 | 功能拆分、实施步骤（含工时估算）、API 合同、代码脚手架/伪代码、测试清单 |
| game-performance | 优化 CPU/GPU、内存、绘制调用、帧率、加载时间 | 性能剖析指南、根本原因假设、具体代码修改建议、性能预算、改动‑测量循环 |

> **注意：** 每个技能还提供了建议的 `scripts/`、`references/`、`assets/` 目录结构（附带示例文件名），方便您以后放置实用脚本、参考文档和模板资源。

## 🛠️ 如何使用

1. **将技能文件夹放入** 您的 Codex skills 目录（默认：`~/.codex/skills/` 或工作区根目录下的 `skill/` 文件夹）。  
   安装后的目录结构示例：
...
2. **在 Codex 对话中调用技能**：  
   ...
   ```
   使用 $game-design-doc 为一款 sci‑fi RPG 起草一份包含三大支柱的 GDD。
   ```

   Codex 会读取对应的 `SKILL.md`，根据需要向您提问澄清信息，然后输出结构化的交付物（默认为 Markdown）。

3. **迭代** – 您可以随时给出反馈或要求展开某一节（例如 “请详细说明盈利模型”），技能将从当前位置继续完善。

4. **添加自有资源** – 按需在每个技能目录下的 `scripts/`、`references/`、`assets/` 中放置实用脚本、参考文档和模板资源，使技能对您的团队更强大。

## 📖 技能文档结构（SKILL.md）

每份 `SKILL.md` 都采用统一的专业模板：

- **YAML 前置信息** – `name` 和详细的 `description`（Codex 触发技能的依据）。  
...
- **## Resources（可选资源）** – 仅在您实际创建对应目录时保留此小节，列出建议放置的文件及其用途，例如：  
  - `scripts/`：`generate_gdd.py`、`balance_calc.py`、`level_template.py`  
  - `references/`：`gdd_structure.md`、`system_design_patterns.md`、`monetization_models.csv`  
  - `assets/`：`gdd_template.docx`、`icon_set/`（里面放 Milestones、Systems、Features 图标）  
...
- **存放参考资料** – 把行业白皮书、设计模式汇总、盈利模型对照表等文件放进 `references/`。  
- **放置模板** – Word/Google Docs 模板、图标集、HTML 骨架等放进 `assets/`。  
- **调整 UI 元信息** – 编辑 `agents/openai.yaml`，修改 `display_name`、`short_description`、`brand_color` 或 `default_prompt`，以便在 Codex 技能选择器中呈现您想要的展示效果。  
- **新增子技能** – 如果您发现经常需要某种特定输出（如 “live-ops 运营计划”），可以参考现有模板新建一个文件夹，按照同样结构编写 `SKILL.md` 与 `openai.yaml`。

## 🚀 快速上手

将本仓库（或直接解压的文件夹）克隆/复制到您的 Codex 目录：

```bash
# 假设您的 Codex 工作区根目录是 ~/codex-workspace
git clone https://github.com/your-username/game-dev-skills.git skills
# 或者手动复制：
cp -r path/to/this/repo/* ~/.codex/skills/
```
...
```bash
# 示例调用
使用 $game-design-doc 为我的回合制策略游戏写一份概念文档。
使用 $game-balance 来平衡武器伤害与敌人血量曲线。
使用 $level-design 设计一个包含三波怪物的地牢关卡。
```
...
> **您**：`使用 $game-design-doc 为一款手机消除类游戏起草一份 GDD，强调“短时段、深度掌握”支柱。`  
> **Codex**：会询问目标受众、平台、核心循环偏好 → 输出一份包含执行摘要、支柱、核心循环表、系统交互矩阵、功能待办列表（P0/P1/P2）、盈利草图、风险日志的 Markdown GDD。

### 平衡战斗系统 / 平衡战斗系统

> **您**：`使用 $game-balance 来平衡近战武器伤害曲线，使 TTD（Time‑to‑Death）保持在 1.5–2.5 秒之间。`  
> **Codex**：请求当前属性、期望的击杀时间、敌人生命曲线 → 生成一份包含公式、缩放曲线和简要解释的电子表格式平衡建议。

### 设计地牢关卡 / 设计地牢关卡

> **您**：`使用 $level-design 设计一个包含三次遭遇的奇幻 roguelike 地牢关卡。`  
...
2. 内部放置 `SKILL.md`（参照上面的模板）以及 `agents/openai.yaml`。  
3. 如需，添加 `scripts/`、`references/`、`assets/` 子目录并说明用途。  
